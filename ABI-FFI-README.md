# EvidenceGraph ABI/FFI Documentation
...
│  generated/abi/evidence_graph.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libevidence_graph.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── evidence_graph.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── evidence_graph.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/evidence_graph.h
```
...
int main() {
    void* handle = evidence_graph_init();
    if (!handle) return 1;

    int result = evidence_graph_process(handle, 42);
    if (result != 0) {
        const char* err = evidence_graph_last_error();
...
Compile with:
```bash
gcc -o example example.c -levidence_graph -L./zig-out/lib
```

### From Idris2

```idris
import EvidenceGraph.ABI.Foreign
...
```rust
#[link(name = "evidence_graph")]
extern "C" {
    fn evidence_graph_init() -> *mut std::ffi::c_void;
    fn evidence_graph_free(handle: *mut std::ffi::c_void);
    fn evidence_graph_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = evidence_graph_init();
        assert!(!handle.is_null());

        let result = evidence_graph_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libevidence_graph = "libevidence_graph"

function init()
    handle = ccall((:evidence_graph_init, libevidence_graph), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:evidence_graph_process, libevidence_graph), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:evidence_graph_free, libevidence_graph), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/evidence_graph.h
   ```
