...
Two-way sync between Zotero and Evidence Graph: - *Export:* Zotero items
→ Evidence Graph (via modified extension) - *Import:* Evidence Graph →
Zotero (via API + manual) - *Metadata:* Preserve Dublin Core,
Schema.org, custom fields
...
The existing bofig repository contains an old Zotero→Voyant export
plugin. We’ll extend it to POST to Evidence Graph API.
...
[arabic]
. User clicks "`Export to Zotero`" in Evidence Graph UI
. Browser downloads `+evidence_123.json+`
. User opens Zotero → File → Import → Select JSON file
. Zotero creates new item with metadata + PROMPT scores in "`Extra`"
field
...
*Last Updated:* 2025-11-22 *Status:* Design complete, implementation
pending
