// EvidenceGraph FFI Implementation
//
...
const VERSION = "0.1.0";
const BUILD_INFO = "EvidenceGraph built with Zig " ++ @import("builtin").zig_version_string;
...
export fn evidence_graph_init() ?*Handle {
...
export fn evidence_graph_free(handle: ?*Handle) void {
...
export fn evidence_graph_process(handle: ?*Handle, input: u32) Result {
...
export fn evidence_graph_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn evidence_graph_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
/// Register a callback
export fn evidence_graph_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn evidence_graph_is_initialized(handle: ?*Handle) u32 {
...
test "error handling" {
    const result = evidence_graph_process(null, 0);
