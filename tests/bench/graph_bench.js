...
// Mechanically transpiled from the former TypeScript benchmark to satisfy the
// estate language policy while preserving Deno benchmark coverage.
...
function createGraphWithNodes(n) {
  const nodes = [];
  for (let i = 0; i < n; i++) {
    nodes.push(
      createNode(
...
function createGraphWithLinks(nodeCount, linkCount) {
  const nodes = [];
...
Deno.bench("Benchmark: Create 1,000 nodes", () => {
  createGraphWithNodes(1e3);
});
Deno.bench("Benchmark: Create 10,000 nodes", () => {
  createGraphWithNodes(1e4);
});
...
Deno.bench("Benchmark: Traverse 1,000 nodes with 5,000 links", () => {
  const graph = createGraphWithLinks(1e3, 5e3);
  let count = 0;
  for (const _link of graph.links) {
    count++;
  }
...
Deno.bench("Benchmark: Traverse 5,000 nodes with 50,000 links", () => {
  const graph = createGraphWithLinks(5e3, 5e4);
  let count = 0;
  for (const _link of graph.links) {
    count++;
  }
...
Deno.bench("Benchmark: Serialize 1,000-node graph", () => {
  const graph = createGraphWithNodes(1e3);
  JSON.stringify(graph);
});
Deno.bench("Benchmark: Serialize 10,000-node graph", () => {
  const graph = createGraphWithNodes(1e4);
  JSON.stringify(graph);
});
...
Deno.bench("Benchmark: Serialize/deserialize 5,000-node graph", () => {
  const graph = createGraphWithNodes(5e3);
  const json = JSON.stringify(graph);
  JSON.parse(json);
});
