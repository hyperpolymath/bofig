...
class EvidenceGraph {
  nodes = [];
  links = [];
  addNode(id, label, nodeType, promptScore) {
...
Deno.test("E2E Tests - Basic Lifecycle", async (t) => {
  await t.step("should create empty graph and verify empty state", () => {
    const graph = new EvidenceGraph();
    assert(graph.isEmpty());
    assertEquals(graph.nodes.length, 0);
    assertEquals(graph.links.length, 0);
  });
  await t.step("should add single node and verify it exists", () => {
    const graph = new EvidenceGraph();
  ...
  await t.step("should add multiple nodes and verify all exist", () => {
    const graph = new EvidenceGraph();
    ...
    const evidence = graph.addNode("evidence_1", "IPCC Report", "evidence", 92);
...
Deno.test("E2E Tests - Link Management", async (t) => {
  await t.step("should add link between two nodes", () => {
    const graph = new EvidenceGraph();
  ...
  await t.step("should reject link with non-existent source node", () => {
    const graph = new EvidenceGraph();
    graph.addNode("claim_1", "Claim", "claim", 75);
    let thrown = false;
    try {
  ...
  await t.step("should reject link with non-existent target node", () => {
    const graph = new EvidenceGraph();
    graph.addNode("evidence_1", "Evidence", "evidence", 90);
    let thrown = false;
    try {
  ...
  await t.step("should allow multiple links to same node", () => {
    const graph = new EvidenceGraph();
    graph.addNode("claim_1", "Claim", "claim", 75);
...
Deno.test("E2E Tests - Serialization & Deserialization", async (t) => {
  await t.step("should serialize empty graph to JSON", () => {
    const graph = new EvidenceGraph();
  ...
  await t.step("should serialize graph with nodes to JSON", () => {
    const graph = new EvidenceGraph();
  ...
  await t.step("should serialize graph with links to JSON", () => {
    const graph = new EvidenceGraph();
    graph.addNode("claim_1", "Claim", "claim", 75);
  ...
  await t.step("should deserialize JSON back to EvidenceGraph", () => {
  ...
  await t.step(
    ...
    () => {
      const original = new EvidenceGraph();
      const _node1 = original.addNode("n1", "Node 1", "claim", 50);
      const node2 = original.addNode("n2", "Node 2", "evidence", 75);
      const _node3 = original.addNode("n3", "Node 3", "claim", 90);
...
Deno.test("E2E Tests - Complete Workflow", async (t) => {
  await t.step("should complete full investigation workflow", () => {
    ...
    investigation.addNode("evidence_ipcc", "IPCC AR6 Report", "evidence", 95);
    investigation.addNode(
      "evidence_nasa",
    ...
    investigation.addLink("evidence_ipcc", "claim_climate_change", "supports");
    investigation.addLink("evidence_nasa", "claim_climate_change", "supports");
    investigation.addLink(
      "evidence_skeptic",
      "claim_climate_change",
      "contradicts",
    );
    investigation.addLink("evidence_ipcc", "claim_human_caused", "supports");
  ...
  await t.step("should handle graph update and re-serialization", () => {
    const graph = new EvidenceGraph();
    ...
    const data1 = JSON.parse(v1);
    const data2 = JSON.parse(v2);
...
Deno.test("E2E Tests - Query Operations", async (t) => {
  ...
  await t.step("should return undefined for non-existent node", () => {
    const graph = setupGraph();
